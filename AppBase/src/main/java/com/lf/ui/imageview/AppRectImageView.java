...
public class AppRectImageView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    public AppRectImageView(Context context) {
        super(context, null);
    }

    public AppRectImageView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs, 0);
    }

    public AppRectImageView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        //取xml文件中设定的参数
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AppRectImageView);
        mIsCircle = ta.getBoolean(R.styleable.AppRectImageView_isCircle, false);
        mBorderColor = ta.getColor(R.styleable.AppRectImageView_borderColor, DEFAULT_BORDER_COLOR);
        mBorderWidth = ta.getDimensionPixelSize(R.styleable.AppRectImageView_borderWidth, dip2px(DEFAULT_BORDER_WIDTH));
        mRectRoundRadius = ta.getDimensionPixelSize(R.styleable.AppRectImageView_rectRoundRadius, dip2px(DEFAULT_RECT_ROUND_RADIUS));
        ta.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (rawBitmap != null) {
            ...
            float dstWidth = mIsCircle ? viewMinSize : viewWidth;
            float dstHeight = mIsCircle ? viewMinSize : viewHeight;
